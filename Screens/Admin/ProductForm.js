...
const ProductForm = (props) => {
  ...
  const route = useRoute(); // Add this line inside your component function
  ...
  useEffect(() => {
    ...
    // Check if route.params.product exists and is not null
    if (route.params && route.params.product) {
      const productData = route.params.product;
      setBrand(productData.brand);
      setName(productData.name);
      setPrice(productData.price.toString()); // Convert price to string
      setDescription(productData.description);
      setRichDescription(productData.richDescription);
      setImage(productData.image);
      setMainImage(productData.image);
      setCategory(productData.category._id); // Assuming category has an _id
      setCountInStock(productData.countInStock.toString()); // Convert countInStock to string
      setRating(productData.rating);
      setNumReviews(productData.numReviews);
      setIsFeatured(productData.isFeatured);
      // You may need to handle image data differently based on your implementation
    }
    ...
    async function requestCameraPermissions() {
      if (Platform.OS !== 'web') {
        ...
        if (status !== 'granted') {
          alert('Sorry, we must have access to choose images');
        }
      }
    }
